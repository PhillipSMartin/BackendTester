...
Logger::Logger(Parameters* pParms) : pParms_(pParms)
{
    if ( pParms_->get_file_log_level() < Parameters::LogLevel::NONE )
    {
        ...
        {
            started_ = Utility::Log::set_log_level( Parameters::LogLevelToStr( pParms_->get_file_log_level() ) ); 
        }
        if ( !started_ )
        {
    ...
    Info( "Logfile log level set to " + Parameters::LogLevelToStr( pParms_->get_file_log_level() ) );
    Info( "Console log level set to " + Parameters::LogLevelToStr( pParms_->get_console_log_level() ) );
    Info( "Tourney id set to " + pParms_->get_tourney_id() );
}
